...
CREATE OR REPLACE FUNCTION os.fn_create_ext_table(p_table text, p_columns text[], p_connection_id int, p_sql_text text)
  RETURNS void AS
$$
DECLARE
...
BEGIN
        ...
        v_location := 4000;
        SELECT os.osjar || ':' || CASE WHEN ms.msjar IS NOT NULL THEN ms.msjar || ':'  ELSE '' END || COALESCE(o.ojar, '') ||
                ' -Xms' || s.xms || ' -Xmx' || x.xmx || ' ExternalData ' || current_database() || ' ' || p.port || ' ' || e.id || 
                ' "' || p_sql_text || '"'
        INTO v_java
        FROM    (SELECT value AS osjar FROM os.variables WHERE name = 'osJar') AS os,
                (SELECT value AS msjar FROM os.variables WHERE name = 'msJar') AS ms,
                (SELECT value AS ojar FROM os.variables WHERE name = 'oJar') AS o,
